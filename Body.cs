...
namespace Змейка
{
    public class Moving
    {
        ...
        {
            if (Program.walls.IsHit(Program.snake.GetHead()) || Program.snake.IsHit(Program.snake.GetHead()))
            {
                Program.time.Change(0, Timeout.Infinite);
            }
            else if (Program.snake.Eat(Program.food.food))
            {
                Program.food.CreateFood();
            }
            else
            {
                Program.snake.Move();
